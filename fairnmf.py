...
def FairNMF_step_MU(Xs, Ws, H, min_errs, norms, coefs):
...
def FairNMF_MU(Xs, num_topics, max_iter = 1000, rel_tol=1e-3):
    '''
    Performs a FairNMF decomposition (multiplicative update).  Xs is a sequence of NumPy arrays each with
    shape (_, d) where the first dimension may differ.  Each array X in Xs gets decomposed into W @ H where
    W has shape (_, num_topics) and H has shape (num_topics, d).  The H array is shared between all X.
...
#            H = model.components_
    ...
    if max_iter is None:
        prev_err = None
        while True:
            Ws, H, coefs, iter_err = FairNMF_step_MU(Xs, Ws, H, min_errs, norms, coefs)
            if not(prev_err is None) and np.max(np.abs(iter_err-prev_err)/(np.abs(iter_err))) < rel_tol:
                #convergence reached
                break
            prev_err = iter_err
        ...
        for i in range(max_iter):
            Ws, H, coefs, iter_err = FairNMF_step_MU(Xs, Ws, H, min_errs, norms, coefs)
            errs[:,i] = iter_err
            if i > 0 and np.max(np.abs(errs[:,i-1]-errs[:,i])/(np.abs(errs[:,i]))) < rel_tol:
                #convergence reached
                break
...
def FairNMF_AM(Xs, num_topics, max_iter = 10, rel_tol=1e-8):
    '''
    Performs a FairNMF decomposition (alternating minimization).  Xs is a sequence of NumPy arrays each with
    shape (_, d) where the first dimension may differ.  Each array X in Xs gets decomposed into W @ H where
    W has shape (_, num_topics) and H has shape (num_topics, d).  The H array is shared between all X.
...
def NMF(X, num_topics, max_iter = 1000, rel_tol=1e-3):
    ...
    for i in range(max_iter):
        ...
        errs[i] = iter_err
        if i > 0 and np.abs(errs[i-1]-errs[i])/(np.abs(errs[i])) < rel_tol:
            #convergence reached
            break
